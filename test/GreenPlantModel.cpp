...
const double GreenPlantModel::A1 = 3;
...
GreenPlantModel::GreenPlantModel(const std::string &csvPath) {
    ...
    #ifdef DEBUG_MODE
    cout<<"[MODELLO PANNELLO] Creato il modello"<<endl;
    #endif  
}
...
int main(){
    ...
    const std::string csvPath = "../test/2019.csv";
    GreenPlantModel model(csvPath);

    double time;
    std::cout << "Enter time in minutes to get produced power: ";
    std::cin >> time;

    try {
        double power = model.getProducedPowerByTime(time);
        std::cout << "Produced power at " << time << " minutes: " << power << " W" << std::endl;
    } catch (const std::out_of_range &e) {
