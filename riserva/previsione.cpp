...
float previsioneEnergiaDisponibile(int n,int min){
...
void inizializzaMatriceTest(GreenPlantModel model){
    for(int i=0;i<DAYS;i++){
        for(int j=0;j<NN;j++){
            double minuti=i*60+j*FREQUENZA;
...
void previsioneDelGiorno(int c){
    for(int i = c;i< NN;i++){
        ...
        matrice_energetica[oggi][i]= alfa * medie_energetiche[i] + (1-alfa)*gapk(i+1)* matrice_energetica[oggi][i-1];
    }
}
